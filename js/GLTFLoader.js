...
THREE.GLTFLoader = ( function () {
	...
	GLTFLoader.prototype = {
		...
		parse: function ( data, path, onLoad, onError ) {
			...
			if ( typeof data === 'string' ) {
				...
				//读取文件头
				var magic = THREE.LoaderUtils.decodeText( new Uint8Array( data, 0, 4 ) );//8位数组4个元素

				if ( magic === BINARY_EXTENSION_HEADER_MAGIC ) {//文件头是"glTF"

					try {

						extensions[ EXTENSIONS.KHR_BINARY_GLTF ] = new GLTFBinaryExtension( data );//用data创建GLTFBinaryExtension
	...
	function GLTFBinaryExtension( data ) {
		...
		if ( this.header.magic !== BINARY_EXTENSION_HEADER_MAGIC ) {//用头文件判断数据类型

			throw new Error( 'THREE.GLTFLoader: Unsupported glTF-Binary header.' );

		} else if ( this.header.version < 2.0 ) {//版本判断

			throw new Error( 'THREE.GLTFLoader: Legacy binary file detected. Use LegacyGLTFLoader instead.' );

		}

		var chunkView = new DataView( data, BINARY_EXTENSION_HEADER_LENGTH );//创建数据块，切掉头文件
		var chunkIndex = 0;

		while ( chunkIndex < chunkView.byteLength ) {

			var chunkLength = chunkView.getUint32( chunkIndex, true );//截取Uint32
			chunkIndex += 4;

			var chunkType = chunkView.getUint32( chunkIndex, true );
			chunkIndex += 4;

			if ( chunkType === BINARY_EXTENSION_CHUNK_TYPES.JSON ) {//如果是json类型，解析为文字

				var contentArray = new Uint8Array( data, BINARY_EXTENSION_HEADER_LENGTH + chunkIndex, chunkLength );
				this.content = THREE.LoaderUtils.decodeText( contentArray );

			} else if ( chunkType === BINARY_EXTENSION_CHUNK_TYPES.BIN ) {//如果是二进制
